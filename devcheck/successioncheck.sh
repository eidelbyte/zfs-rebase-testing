#!/bin/sh
# Run the set-delta succession rules over the op3 corpus and
# check its believed moves against the fixtures' hand-worked answers.
#
#   devcheck/successioncheck.sh
#   DEMO=/path/to/decide-demo devcheck/successioncheck.sh
#   JSC=/path/to/jsc devcheck/successioncheck.sh
#
...
CORPUS=$(ls "$ROOT"/trees/open-problems/op6-*.tree 2>/dev/null || true)
if [ -z "$CORPUS" ]; then
...
( cd "$DEMO" && "$JSC" jsc-shim.js "$ROOT/devcheck/successioncheck.js" \
    -- $CORPUS )

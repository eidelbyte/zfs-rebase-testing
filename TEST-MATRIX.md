...
| CP14 | SA-only source, dst xattr=dir | converts to a hidden directory; directory stamped 0041777 + ZFS_XATTR + links 2 + size 2+n, children stamped regular + ZFS_XATTR | covered: test_apply_xattr_dir_form (destination set to xattr=dir EXPLICITLY -- the first run caught that the upstream DEFAULT is now sa, not dir; xattr fixtures never trust defaults; the sa-source half rides rt_set_dxattr fixtures when CP15 lands) |
